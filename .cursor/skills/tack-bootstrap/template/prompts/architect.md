...
ADR location policy: new ADRs land in `project/docs/adr/`. If your repo has a legacy `docs/adr/` (or any other historical ADR folder), document that in **`TACK.md`** and never create new ADRs there — reference legacy ADRs by id only. Continue ADR numbering across **all** ADR folders.
...
3. **Invariants:** If **`TACK.md`** defines **parity**, **identity**, or **naming** rules that apply to specific modules, tasks that touch those modules must **explicitly** call out those invariants in the task description.
