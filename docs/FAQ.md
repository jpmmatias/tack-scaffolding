...
- Run worktree creation from a normal terminal with full git access to the repo (not a restricted sandbox), or adjust **`TACK.md`** keys `tack.worktree.*` (e.g. `tack.worktree.mode`, `tack.worktree.dir`) as documented in that orchestrator prompt.
...
- [`tack-doctor.sh`](../skills/tack-bootstrap/template/scripts/tack-doctor.sh) fails if repo-root **`TACK.md`** (default) or **`--rules`** path still contains `<UPPERCASE_PLACEHOLDER>` tokens or if **`Specialist routing`** in `project/prompts/auto-orchestrator.md` still has `<fill>` rows (template ships empty until you customize).
...
- Remove placeholder tokens from **`TACK.md`** per Phase 5 of [`SKILL.md`](../skills/tack-bootstrap/SKILL.md).
