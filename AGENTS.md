...
- [`skills/tack-agent/`](skills/tack-agent/) — Invoke a **single** Tack prompt under `project/prompts/` (PM, architect, QA, worker, reviewer, security, worktree coordinator, or a specialist). Use when you need one step only; use `tack-run` for the full pipeline.
