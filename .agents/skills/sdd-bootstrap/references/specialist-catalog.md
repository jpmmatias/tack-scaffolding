...
Reference catalog for **Phase 4** of `sdd-bootstrap`. Every entry below is a *candidate*; the skill must surface them as a checklist (per Phase 4 of `SKILL.md`) and only create the ones the user explicitly checks.

Conventions:

- **Suggested model** uses the orchestrator tag — `[Composer]` (mechanical), `[Sonnet]` (contractual), `[Opus]` (high-stakes reasoning). Maps to the slugs documented in `project/prompts/auto-orchestrator.md`.
