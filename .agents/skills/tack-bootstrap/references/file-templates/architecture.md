...
- Session lookup: <e.g. cookie + DB session>. Cite the resolution order in repo-root **`TACK.md`** (canonical); legacy repos cite **`.cursorrules`** only if **`TACK.md`** is absent.
...
- The **Layers / boundaries** table seeds the boundary rules in **`TACK.md`**. Keep them aligned with `project/docs/architecture.md`; if you still maintain **`.cursorrules`** for Cursor hints only, avoid duplicating `tack.worktree.*` / routing keys there when **`TACK.md`** exists.
