...
12. **Agent-driven scaffolding.** Surface-specific writes (**`.cursorrules`** stub only when Cursor is active, `.cursor/skills/`, `CLAUDE.md`, `.claude/skills/`, `AGENTS.md`, `.agents/skills/`) are governed by **`tack.agents.active`** — the explicit set of agents the user confirmed in Phase 1. **`TACK.md` at the repo root is independent of agent surface** — always emit it when the user accepts Phase 5 (filled from `project/TACK.md.template`). Allowed values: `claude-code`, `cursor`, `copilot`, `codex`, `antigravity`. **Never write to a surface directory unless its agent appears in `tack.agents.active`.** A leftover `.cursor/` does not authorize Cursor scaffolding — only an explicit user confirmation does. The legacy `tack.routing.surfaces` is **derived** from `tack.agents.active` (see Phase 1) and only chooses between `claude` / `agents` / `both` / `none` for the splice in step 3b. Splice the `## Tack routing` H2 only — never overwrite other sections. Defaults: `auto = yes`.
...
- Agent surfaces detected:
    ...
    Cursor        : TACK.md / .cursorrules / .cursor/      present | absent
...
Confirm or correct any field above before I proceed. **In particular, confirm or correct `tack.agents.active`** — list every AI coding agent you actively use in this repo, drawn from {`claude-code`, `cursor`, `copilot`, `codex`, `antigravity`}. If you use only one, say so explicitly. I will only scaffold surface files (**optional `.cursorrules` stub** when `cursor` is active, `CLAUDE.md`, `AGENTS.md`, `.claude/skills/`, `.cursor/skills/`, `.agents/skills/`) for the agents you confirm — a leftover directory does not authorize scaffolding. **`TACK.md`** is written for every bootstrap regardless of IDE. Reply with corrections, or "correct" to accept.
```
...
3. **`TACK.md`** — **always** (independent of `tack.agents.active`). At the consumer repo root, derived from `project/TACK.md.template` (from step 1). Replace every `<PLACEHOLDER>` with values gathered in Phases 1–3. Fill **Parallel execution (worktrees)** from Block F (`tack.worktree.mode`, `tack.worktree.naming`, `tack.worktree.base`, `tack.worktree.dir`) and routing keys from Phase 1. Use `references/file-templates/tack.md` as the worked shape.
   **Optional `.cursorrules` stub:** **only when `cursor` ∈ `tack.agents.active`** — write repo-root `.cursorrules` from `project/.cursorrules.template` (Cursor-specific editor hints only; see `references/file-templates/cursorrules.md`). Do **not** duplicate `<TEST_COMMAND>`, `tack.worktree.*`, or `tack.routing.*` here unless the team is migrating from legacy `.cursorrules`-only. If Cursor is **not** active, skip the stub — `project/.cursorrules.template` still ships under `project/` for future adoption. **`TACK.md` alone satisfies bootstrap** in that case: Phase 6 does not require repo-root `.cursorrules` for completion, and the template file remains reference-only until Cursor is added to `tack.agents.active`.
...
1a. Run **`bash project/scripts/tack-doctor.sh`** from the consumer repo root. It fails if the **resolved rules file** (repo-root **`TACK.md` preferred**, else legacy **`.cursorrules`**) still contains `<UPPERCASE_PLACEHOLDER>` tokens, if **`TACK.md`** exists **and** a companion **`.cursorrules`** still has those tokens (same regex; **`TACK.md` stays canonical** for orchestration — use `--rules PATH` to validate only that file for placeholders), if `project/prompts/auto-orchestrator.md` still has `<fill>` rows in the **Specialist routing** table, or if **`tack.routing.auto`** is **not** explicitly `no` (see `project/TACK.md.template` — default is **yes** when absent) while **`project/docs/tack-pipeline-models.md`** is missing or its YAML front matter lacks a required pipeline key. If both **`TACK.md`** and **`.cursorrules`** exist, the script warns that **`TACK.md` is authoritative**. If it reports issues, return to Phase 5 step 3 (**`TACK.md`** / optional `.cursorrules`), step 7 (Specialist routing), or step **1b** (pipeline models) and fix before continuing.
2. Print the SDD 7-step pipeline as a final checklist:

   ```text
   0. [ ] (optional) @project/prompts/worktree-coordinator.md — isolated worktree + branch per `tack.worktree.*` in **`TACK.md`** (canonical); legacy repos may use the same keys only in **`.cursorrules`** when **`TACK.md`** is absent
...
- `references/file-templates/tack.md` — anonymized worked example for **`TACK.md`**.
- `references/file-templates/cursorrules.md` — optional Cursor **`.cursorrules`** stub (points at **`TACK.md`**).
...
- `template/scripts/tack-resolve-config.sh` — resolves **`TACK.md`** vs **`.cursorrules`** for `tack-worktree.sh` / `tack-doctor.sh`; copied to `project/scripts/`.
...
- `template/scripts/tack-doctor.sh` — Phase 6 step 1a verification; fails on leftover `<UPPERCASE_PLACEHOLDER>` tokens in the resolved rules file (**`TACK.md`** default, else **`.cursorrules`**), and in repo-root **`.cursorrules`** when **`TACK.md`** is the primary file (unless `--rules` is set — then only that path is checked for placeholders), `<fill>` rows in `project/prompts/auto-orchestrator.md` **Specialist routing**, and incomplete **`project/docs/tack-pipeline-models.md`** when auto-orchestration is active. Warns when both **`TACK.md`** and **`.cursorrules`** exist. Copied to `project/scripts/` for ongoing post-bootstrap checks.
