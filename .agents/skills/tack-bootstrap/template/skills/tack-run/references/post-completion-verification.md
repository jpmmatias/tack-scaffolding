...
> **Scope note.** This is the **post-pipeline** check emitted as `Implementation verification: …` in the Final report. It is **not** the same as `tack-agent`'s per-dispatch `Verification: …` line (single-subagent scope). Same `PASS | GAP | FAILED` vocabulary, different label and scope. Do **not** emit bare `Verification:` here.
...
**Outcome:** Emit exactly one line in the Final report:

`Implementation verification: PASS | GAP | FAILED — <short narrative>`

Narrative covers user ask ↔ spec AC coverage, commands run with exit status, and any notable diff observation. Enumerate missing items under **GAP**/**FAILED**.
