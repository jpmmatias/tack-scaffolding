...
**Orchestrator-specific operational note (after Step −1 succeeds):** always pass **`working_directory` / `cwd` *and* the first **INPUTS** line `cd <absolute worktree_path> &&`** (or equivalent) on **every** Step 1–7 / 7b dispatch — some hosts ignore the parameter on the first dispatch.
