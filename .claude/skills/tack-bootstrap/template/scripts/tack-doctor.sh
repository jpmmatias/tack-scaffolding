...
#      <UPPERCASE_PLACEHOLDER> tokens (matches `<[A-Z][A-Z0-9_]*>`).
...
Validates a bootstrapped Tack repo:
  1. <UPPERCASE_PLACEHOLDER> tokens removed from TACK.md (or --rules / fallback .cursorrules).
...
Defaults:
  --rules         TACK.md at repo root if present, else .cursorrules (override with --rules PATH)
