...
- Test files using your project’s conventions (suffixes, colocation rules — read from **`TACK.md`** / **`.cursorrules`**; common patterns: `**/*.test.ts`, `**/*.test.tsx`, `tests/**`)
...
2. Run the test using **`<TEST_COMMAND>`** from **`TACK.md`** / **`.cursorrules`** and capture **red** output. Paste or summarize the failure in your reply **before** asking `@worker.md` to implement.
