...
1a. Run **`bash project/scripts/tack-doctor.sh`** from the consumer repo root. It fails if **`TACK.md`** (or `--rules` override / fallback `.cursorrules`) still contains `<UPPERCASE_PLACEHOLDER>` tokens, if `project/prompts/auto-orchestrator.md` still has `<fill>` rows in the **Specialist routing** table, or if **`tack.routing.auto`** is **not** explicitly `no` (see `project/TACK.md.template` — default is **yes** when absent) while **`project/docs/tack-pipeline-models.md`** is missing or its YAML front matter lacks a required pipeline key. If both **`TACK.md`** and **`.cursorrules`** exist, the script warns that **`TACK.md` is authoritative**. If it reports issues, return to Phase 5 step 3 (**`TACK.md`** / optional `.cursorrules`), step 7 (Specialist routing), or step **1b** (pipeline models) and fix before continuing.
2. Print the SDD 7-step pipeline as a final checklist:

   ```text
   0. [ ] (optional) @project/prompts/worktree-coordinator.md — isolated worktree + branch per `tack.worktree.*` in **TACK.md** / **`.cursorrules`**
...
- `template/scripts/tack-doctor.sh` — Phase 6 step 1a verification; fails on leftover `<UPPERCASE_PLACEHOLDER>` tokens in **`TACK.md`** (default) or `--rules` path, `<fill>` rows in `project/prompts/auto-orchestrator.md` **Specialist routing**, and incomplete **`project/docs/tack-pipeline-models.md`** when auto-orchestration is active. Warns when both **`TACK.md`** and **`.cursorrules`** exist. Copied to `project/scripts/` for ongoing post-bootstrap checks.
