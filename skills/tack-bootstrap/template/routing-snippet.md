...
- **Configuration:** read `<TEST_COMMAND>`, `<LINT_COMMAND>`, parity invariants, and `tack.worktree.*` from repo-root **`TACK.md`** only.
- **Worktrees:** honor `tack.worktree.mode`/`naming`/`base`/`dir`/`cleanup` from **`TACK.md`**. The orchestrator decides whether to isolate and whether to offer end-of-run cleanup; do not bypass it. `tack-worktree.sh remove` refuses protected branches (`main`, `master`, `develop`, `staging`, `release/*`, `hotfix/*`, …) unconditionally.
