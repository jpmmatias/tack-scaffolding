...
- **Computational** sensors are deterministic (CPU, fast). **Inferential** sensors (e.g. optional LLM review) are slower and probabilistic—they **never** replace hard failures on invariants defined in **`TACK.md`** or the spec.
...
2. **Architecture fitness** — ADRs, module boundaries, stack-specific invariants from **`TACK.md`**.
