...
- Repository rules file: [project/TACK.md.template](../TACK.md.template) → **`TACK.md`** at repo root (canonical); if missing, **`.cursorrules`** — especially `<TEST_COMMAND>`, `<TEST_HARNESS_ROOT>` / harness paths, lint commands, and project layout conventions.
