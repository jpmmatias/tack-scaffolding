...
1. [ ] **`[Opus]`** (`product_manager`) `@product-manager.md` — write `specs/S-XXX-<slug>.md` (interactive loop: optional clarification questions, mandatory **confirm-before-write** `[CONFIRM_SPEC]` turn in `mode: autonomous`, then the spec)
