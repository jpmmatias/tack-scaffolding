...
ADR location policy: new ADRs land in `project/docs/adr/`. If your repo has a legacy `docs/adr/` (or any other historical ADR folder), document that in **`TACK.md`** / **`.cursorrules`** and never create new ADRs there — reference legacy ADRs by id only. Continue ADR numbering across **all** ADR folders.
...
3. **Invariants:** If **`TACK.md`** / **`.cursorrules`** defines **parity**, **identity**, or **naming** rules that apply to specific modules, tasks that touch those modules must **explicitly** call out those invariants in the task description.
...
8. **DDD plan annotations (DDD profile only):** when the glossary has a `## Bounded contexts` section, `plan.md` MUST list — under a `## DDD impact` heading — the affected aggregates (read vs. mutate), the domain events emitted by this change (with `<PastTenseVerb><Aggregate>` names), and the invariants enforced or changed. **Cross-context calls** require an explicit ACL location pinned in the plan (path under `src/**`); if no ACL exists yet, that's an ADR (rule 4). When the glossary has no `## Bounded contexts` section, rule 8 is a no-op.
