...
- Repository rules file (e.g. [project/TACK.md.template](../TACK.md.template) → **`TACK.md`** at repo root; fallback **`.cursorrules`**)
...
1. **`src/**` or production tree without tests** — Any behaviour change under the application tree that does **not** add or modify at least one test file? → **FAIL** if yes (unless task explicitly documents why tests are impossible—rare). *(Adjust `src/**` in this rule to your repo layout in **`TACK.md`** / **`.cursorrules`** if needed.)*
...
6. **Boundary mocks** — Bespoke mocks for boundaries that **must** use shared harness doubles (per **`TACK.md`** / **`.cursorrules`** / [test-harness.md](../docs/test-harness.md))? → **FAIL** if yes.
...
**DDD (only when **`TACK.md`** / **`.cursorrules`** contains the `Bounded contexts and ubiquitous language` section — otherwise skip)**
...
## Project-specific invariants (fill in **`TACK.md`** / **`.cursorrules`** and mirror here)
