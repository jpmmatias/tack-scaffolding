# File template — `TACK.md`

Worked example, **anonymized and trimmed** from the OrderFlow sample. Use as a shape guide; replace every `<PLACEHOLDER>` and every concrete OrderFlow value with what you collected during Phases 1–3 of `tack-bootstrap`. Save as **`TACK.md`** at the **repository root** (not under `project/`). The bundled shape also ships as **`project/TACK.md.template`** in the consumer repo after bootstrap.
...
- Do not write `<TBD>` or `???` into **`TACK.md`**. If a value is unknown, **ask** before generating the file.
...
- If the consumer repo already has **`TACK.md`**, **diff** before overwriting and offer merge.
