...
Worked example, **anonymized and trimmed** from the OrderFlow sample. Use as a shape guide; replace every `<PLACEHOLDER>` and every concrete OrderFlow value with what you collected during Phases 1–3 of `sdd-bootstrap`. Save as `.cursorrules` at the **repository root** (not under `project/`).
