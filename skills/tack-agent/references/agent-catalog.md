...
|-------|------|-------|--------------|----------------------------|
| Worktree coordinator | `worktree-coordinator.md` | `[Composer]` | `worktree_coordinator` | Isolated branch/worktree; needs slug, `tack.worktree.*` from repo-root TACK.md (canonical) or legacy .cursorrules; OUTPUTS: JSON only |
